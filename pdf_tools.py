        ...
        for page in writer.pages:
            if "/Resources" in page and "/XObject" in page["/Resources"]:
                ...
                for obj_name in xobjects:
                    ...
                    if obj["/Subtype"] == "/Image":
                        try:
                            pass  # Compression handled by writer
                        except:
                            pass
...
async def unlock_pdf(password: str = Form(...), files: List[UploadFile] = File(...)):
...
async def watermark_pdf(text: str = Form(...), files: List[UploadFile] = File(...)):
    """Add text watermark to PDF (basic implementation)"""
    try:
        file = files[0]
        content = await file.read()
        reader = PdfReader(BytesIO(content))
        ...
        # Full watermark implementation would require reportlab
